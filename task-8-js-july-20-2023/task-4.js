...
console.log('__________________task 4___________________');
...
const me = {
  ...
  age: 25,
  date: '10/2/1998',
  favoriteFood: ['macaroni', 'Mansaf', 'Pizza'],
  favoriteMovie: ['avatar', 'fast and furious', 'world war z', 'onepiece', 'demon slayer',]
}
...
function firstName(arr) {
  ...
  const fname = arr.map(function (names) {
...
function ava(obj) {
  const average = obj.reduce(function (num, person) {
    return num + person.age 
  }, 0)
  return average / persons.length
}

console.log(ava(persons));
...
const namess = persons.reduce(function (acc, curr) {
  const fullName = (`${curr.name.first} ${curr.name.last}`);
  acc.push(fullName);
  return acc;
}, []);

const longestNames = namess.reduce(function (long, sort) {
...
const text = "My name is alex mercer class name B baba mama hello Hello HELLO"
